...
function chooseBestSum(t, k, ls) {
    ...
    var maxSum = 0;
    var numberOfCitiesToVisit = k;
    var milesLimit = t;
    var optionsArray = [];
    if (numberOfCitiesToVisit > ls.length) {
        return null;
    }
    for (var index = 0; index <= ls.length - numberOfCitiesToVisit; index++) {
        var firstNumber = ls[index];
        for (var y = index + 1; y < ls.length; y++) {
            var secondNumber = ls[y];
            for (var i = y + 1; i < ls.length; i++) {
                var subArray = [];
    ...
    for (var index = 0; index < optionsArray.length; index++) {
        var currentOptionArray = optionsArray[index];
        var currentSum = 0;
        for (var pipi = 0; pipi < currentOptionArray.length; pipi++) {

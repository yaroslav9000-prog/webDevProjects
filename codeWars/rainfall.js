...
abs(your_result - test_result) <= 1e-2 or abs((your_result - test_result) / test_result)
<= 1e-6 depending on the language.
...
exports.variance = exports.mean = void 0;
...
var intoObject = function (data) {
    var dataArray = data.split('\n').map(function (item) { return (item.substring(item.indexOf(":") + 1)); });
    var citiesArray = data.split('\n').map(function (item) { return (item.substring(0, item.indexOf(":"))); });
    var dataObj = {};
    for (var i = 0; i < dataArray.length; i++) {
        var months = dataArray[i].split(",").map(function (item) { return item.split(" "); });
        dataObj[citiesArray[i]] = months;
    }
    return dataObj;
};
///////////////////////////////////////////////////////////
var mean = function (town, s) {
    var dataObj = intoObject(s);
    if (dataObj[town] === undefined) {
        return -1;
    }
    var theSum = 0;
    for (var i = 0; i < dataObj[town].length; i++) {
        theSum += +dataObj[town][i][1];
    }
    return theSum / 12;
};
exports.mean = mean;
var variance = function (town, strng) {
    var dataObj = intoObject(strng);
    if (dataObj[town] === undefined) {
        return -1;
    }
    var theMean = (0, exports.mean)(town, strng);
    var deviationArray = dataObj[town].map(function (item) { return (Math.pow(item[1] - theMean, 2)); }).reduce(function (accumulator, currentValue) { return accumulator + currentValue; }, 0) / 12;
    return deviationArray;
};
exports.variance = variance;
console.log((0, exports.mean)('London', data));
console.log((0, exports.variance)('London', data));

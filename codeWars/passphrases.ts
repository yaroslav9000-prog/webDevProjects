...
export function playPass (s: string, n: number): any{
    ...
    for(let index = 0; index< splitString.length; index++){
        ...
        if(myIndex.charCodeAt(0) >= 97 && myIndex.charCodeAt(0) <= 122){
            if(myIndex.charCodeAt(0) + n > 122){
                const theDiff = myIndex.charCodeAt(0) + n - 122;
    ...
    for(let counter = 0; counter < NanArray.length; counter++){
        let oddCheck = counter + 1;
        if(oddCheck % 2 == 0){

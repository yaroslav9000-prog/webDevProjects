...
export const mean= (town:string, s: string):number=>{
    const dataObj = intoObject(s);
    if(dataObj[town] === undefined){
        return -1;
    }
    let theSum: number = 0;
    for(let i =0; i< dataObj[town].length; i++){
...
export const variance = (town: string, strng: string) =>{
    const dataObj = intoObject(strng);
    if(dataObj[town] === undefined){
        return -1;
    }
